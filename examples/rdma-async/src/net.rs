...
impl RdmaConnection {
    ...
    pub async fn send(&self, buf: Buf, nbytes: usize) -> (Result<()>, Buf) {
        work::send(self.qp.clone(), buf, nbytes).await
    }

    pub async fn recv(&self, buf: Buf) -> (Result<usize>, Buf) {
        work::recv(self.qp.clone(), buf).await
    }
}
